...
(defclass PrecisionPlacementTest3 (is-a TestScenario) (role concrete))
...
(defmessage-handler PrecisionPlacementTest3 setup (?time ?state-machine)
  (make-instance [stopped-state] of StoppedState
    (phase EXECUTION) (state-machine ?state-machine) (time ?time))
  (make-instance [running-state] of RunningState
    (phase EXECUTION) (state-machine ?state-machine) (time ?time) (max-time ?*PPT-TIME*))
...
(defrule init-ptt
  ...
  ?bm <- (object (is-a Test))
  =>
  (make-instance [PPT1] of PrecisionPlacementTest1 (type PPT) (type-id 1) (description "Prepare Assembly Aid Tray"))
  (make-instance [PPT3] of PrecisionPlacementTest3 (type PPT) (type-id 3) (description "Fill a Box with Parts for Manual Assembly"))

  (slot-insert$ ?bm registered-scenarios 1 [PPT1])
  (slot-insert$ ?bm registered-scenarios 1 [PPT3])
)
