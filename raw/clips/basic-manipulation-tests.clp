...
(defclass BasicManipulationTest3 (is-a TestScenario) (role concrete))
...
(defmessage-handler BasicManipulationTest1 handle-feedback (?pb-msg ?time ?name ?team)
  (if (and
   ...
   then
    (return CONTINUE)   ; BMT feedback is valid -> continue the test
   else
...
(defmessage-handler BasicManipulationTest3 setup (?time ?state-machine)
  (make-instance [stopped-state] of StoppedState
    (phase EXECUTION) (state-machine ?state-machine) (time ?time))
  (make-instance [running-state] of RunningState
    (phase EXECUTION) (state-machine ?state-machine) (time ?time) (max-time ?*BMT-TIME*))
...
(defrule init-bmt
  ...
  (make-instance [BMT3] of BasicManipulationTest3 (type BMT) (type-id 3) (description "Fill a Box with Parts for Manual Assembly"))

  (slot-insert$ ?bm registered-scenarios 1 [BMT1])
  (slot-insert$ ?bm registered-scenarios 1 [BMT3])
)
